...
def init_logging():
  ...
  try:
    ...
    if getattr(config, "VERBOSE_LOGGING", False):
      ...
      logging.getLogger().info("[LOG] Verbose logging enabled via config.")

    if getattr(config, "DEVICE_DEBUG_LOGGING", False):
      args.device_debug = True
      logging.getLogger().info("[LOG] Device debug logging enabled via config.")

    if getattr(config, "SAVE_DEBUG_IMAGES", False):
      global SAVE_DEBUG_IMAGES
      SAVE_DEBUG_IMAGES = True
      args.save_images = True
      logging.getLogger().info("[LOG] Saving debug images enabled via config.")
  except Exception:
    # Avoid hard failure if config is not available during logger init
    pass
